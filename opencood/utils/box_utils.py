...
def corner_to_center(corner3d, order='lwh', whole_pose=False):
    """
    Convert 8 corners to x, y, z, dx, dy, dz, yaw.
    or x, y, z, roll, yaw, pitch, dx, dy, dz
    ...
    if order == 'lwh':
        return np.concatenate([xyz, l, w, h, theta], axis=1).reshape(
            batch_size, 7) if not whole_pose else \
            np.concatenate([xyz, roll, yaw, pitch, l, w, h], axis=1).reshape(
            batch_size, 9)
    elif order == 'hwl':
