...
def compute_distance(box_center, boxes_center):
    # calculate the distance between target box and boxes
    distance = np.sqrt(np.sum((boxes_center[:, :2] - box_center) ** 2, axis=1))
    return distance
...
def interpolate_missing_points(trajs, trajs_mask):
    """
    linear interpolation of missing points
    Parameters
    ----------
    trajs: np.ndarray
        containing trajectory data, and shape is [num_objects, frames, dims]

    trajs_mask:np.ndarray
    ...
    _, num_frames, num_dims = trajs.shape
    all_frames = np.arange(num_frames)
    object_indices = np.unique(np.nonzero(trajs_mask)[0])

    for object_index in object_indices:
        available_frames = np.nonzero(trajs_mask[object_index, :])[0]
        # Interpolation requires two available points at least
        # Otherwise, directly copy the current point
        if len(available_frames) < num_frames:
            for dim in range(num_dims):
                if len(available_frames) > 1:
                    available_value = trajs[object_index, available_frames, dim]
                    trajs[object_index, :, dim] = \
                        np.interp(all_frames, available_frames, available_value)
                else:
                    trajs[object_index, :, dim].fill(
                        trajs[object_index, available_frames[0], dim])

            trajs_mask[object_index, :] = 1

    return trajs, trajs_mask
